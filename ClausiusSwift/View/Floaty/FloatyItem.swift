...
open class FloatyItem: UIView {
     ...
     */
    @objc open var size: CGFloat = 40.0 {
        didSet {
            ...
            _innerTitleLabel?.center = CGPoint(x: size/2, y: size/2)
    ...
    /**
     Item's inner label.
     */
    var _innerTitleLabel: UILabel? = nil
    @objc open var innerTitleLabel: UILabel {
        get {
            if _innerTitleLabel == nil {
                _innerTitleLabel = UILabel(frame: CGRect(x: 0, y: 0, width: imageSize.width, height: imageSize.height))
                _innerTitleLabel?.center = CGPoint(x: size/2, y: size/2) + imageOffset
                _innerTitleLabel?.contentMode = UIView.ContentMode.scaleAspectFill
                _innerTitleLabel?.backgroundColor = .clear
                addSubview(_innerTitleLabel!)
            }
    ...
    /**
     Item's inner label text.
     */
    @objc open var innerTitle: String? = nil {
        didSet {
            innerTitleLabel.text = innerTitle
            if let font = innerTitleLabel.font {
                let labelSize = (innerTitle! as NSString).size(withAttributes: [NSAttributedString.Key.font:font])
                _innerTitleLabel?.frame = CGRect(x: 0.0, y: 0.0, width: labelSize.width, height: labelSize.height)
                _innerTitleLabel?.center = CGPoint(x: size/2, y: size/2)
            }
        }
    }

    @objc open var innerTitleFont: UIFont? = nil {
        didSet {
            innerTitleLabel.font = innerTitleFont
            if let text = innerTitleLabel.text {
                let labelSize = (text as NSString).size(withAttributes: [NSAttributedString.Key.font:innerTitleFont!])
                _innerTitleLabel?.frame = CGRect(x: 0.0, y: 0.0, width: labelSize.width, height: labelSize.height)
                _innerTitleLabel?.center = CGPoint(x: size/2, y: size/2)
            }
        }
    ...
    open override func draw(_ rect: CGRect) {
        ...
        if _innerTitleLabel != nil {
            bringSubviewToFront(_: innerTitleLabel)
        }
    }
